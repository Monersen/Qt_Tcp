 ...
 */
    //Соединение для получения данных времени от сервера
    connect(client, &TCPclient::sig_sendTime, this, &MainWindow::DisplayTime);
    //Соединение для запуска метода вывода статуса подключения к серверу
    connect(client, &TCPclient::sig_connectStatus, this, &MainWindow::DisplayConnectStatus);
    //Соединение для получения данных о свободном месте на сервере
    connect(client, &TCPclient::sig_sendFreeSize, this, &MainWindow::DisplayFreeSpace);
    //Соединение для получения статистики от сервера
 ...
 */
//Метод вывода времени в виджет
void MainWindow::DisplayTime(QDateTime time)
{
    ui->tb_result->append("Current time and date on the server: -> " + time.toString());
}
//Метод вывода свободного места на сервере в виджет
void MainWindow::DisplayFreeSpace(uint32_t freeSpace)
{
    ui->tb_result->append("Free space on the server: -> " + QString::number(freeSpace));
}
void MainWindow::SetDataReply(QString replyString)
{

}
//Метод вывода статистики сервера в виджет
void MainWindow::DisplayStat(StatServer stat)
{
    ui->tb_result->append("Bytes received: -> " + QString::number(stat.incBytes));
    ui->tb_result->append("Bytes passed: -> " + QString::number(stat.sendBytes));
    ui->tb_result->append("Accepted packages: -> " + QString::number(stat.revPck));
    ui->tb_result->append("Packets transmitted: -> " + QString::number(stat.sendPck));
    ui->tb_result->append("Server running time (seconds): -> " + QString::number(stat.workTime));
    ui->tb_result->append("Number of connected clients: -> " + QString::number(stat.clients));
}
